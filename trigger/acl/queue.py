...
class BaseQueue(object):
    ...
    def _get_firewall_db_conn(self):
        """
        Returns a db connection for the ACL queues using database settings found
        within ``settings.py``.
        """
        raise NotImplementedError("You define this method in your subclass!")
        ...
        if len(routers):
            for router in routers:
                try:
                    ...
                    raise exceptions.TriggerError("Could not find %s in netdevices" % router)
                    return
                if acl not in dev.acls:
                    raise exceptions.TriggerError("Could not find %s in %s's acl list" % (acl, router))
                    return
    ...
    def delete(self, acl, routers=None, escalation=False):
        ...
        if len(devs):
            for dev in devs:
                self.cursor.execute('''delete from acl_queue where acl=? and
                                    router=? and loaded is null''', (acl, dev))
    ...
    def complete(self, device, acls):
        ...
        """
        for acl in acls:
            self.cursor.execute("""update acl_queue set loaded=datetime('now') where
                                acl=? and router=? and loaded is null""",
                                (acl, device))
        if self.verbose:
    ...
    def remove(self, acl, routers, escalation=False):
        ...
        assert acl, 'no ACL defined'
        for router in routers:
            self.cursor.execute("""update acl_queue set loaded=0 where acl=?
                                and router=? and loaded is null""", (acl, router))
        if self.verbose:
...
class SQLiteQueue(BaseQueue):
    def _get_firewall_db_conn(self):
        """Returns a SQLite db connection used for the ACL queues using database
        settings found withing ``settings.py``."""
        if sqlite3 is None:
            raise RuntimeError("You must have ``sqlite3`` to use the queue")
        try:
            return sqlite3.connect(database=settings.DATABASE_HOST)
