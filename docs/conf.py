...
def issues_role(name, rawtext, text, lineno, inliner, options=None, content=None):
    """
    ...
    if name in issue_types:
        which = f'[<span class="changelog-{name}">{name.capitalize()}</span>]'
        ret = [
...
project = 'Trigger'
copyright = f'2006-{datetime.now().year}, AOL Inc'
...
latex_documents = [
  ('index', 'Trigger.tex', 'Trigger Documentation',
   'Jathan McCollum, Eileen Tschetter, Mark Ellzey Thomas, Michael Shields', 'manual'),
]
