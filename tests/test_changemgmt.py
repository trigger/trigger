...
class CheckBounceStatus(unittest.TestCase):
    ...
    def testComparison(self):
        ...
        self.assertTrue(self.red > self.yellow > self.green)
        self.assertTrue(self.red == self.red == BounceStatus("red"))
        self.assertNotEqual(self.red, self.yellow)

    def testString(self):
        """Test BounceStatus stringfication and string comparison."""
        self.assertEqual(str(self.red), "red")
        self.assertEqual(self.red, "red")
        self.assertTrue("red" > self.yellow > "green")
...
class CheckBounceWindow(unittest.TestCase):
    ...
    def testStatus(self):
        ...
        self.assertEqual(self.eastern.status(when), "red")
        # 03:00 PST, 14:00 UTC
        then = pacific.localize(datetime(2013, 6, 4))
        self.assertEqual(self.pacific.status(then), "green")
    ...
    def testNextOk(self):
        ...
        self.assertEqual(next_ok.tzinfo, UTC)
        self.assertEqual(next_ok.astimezone(eastern).hour, 2)
        self.assertEqual(next_ok, datetime(2013, 1, 4, 7, 0, tzinfo=UTC))
        self.assertEqual(self.pacific.status(next_ok), "green")
        # next_ok() should return current time if already ok.
        self.assertEqual(self.pacific.next_ok("yellow", next_ok), next_ok)
        then = datetime(2013, 1, 3, 22, 15, tzinfo=UTC)
        self.assertEqual(self.pacific.next_ok("red", then), then)
...
class CheckWeekend(unittest.TestCase):
    def testWeekend(self):
        ...
        self.assertEqual(next_ok, datetime(2006, 1, 9, 10, tzinfo=UTC))
...
class CheckNetDevices(unittest.TestCase):
    ...
    def testNetDevicesBounce(self):
        ...
        self.assertEqual(self.router.bounce.status(self.when), "red")

    def testAllowability(self):
        """Test allowability checks."""
        self.assertFalse(self.router.allowable("load-acl", self.when))
        morning = datetime(2006, 7, 25, 9, tzinfo=UTC)  # 5 am EDT
        self.assertTrue(self.router.allowable("load-acl", morning))
        self.assertEqual(self.router.next_ok("load-acl", self.when), morning)
        self.assertEqual(self.router.next_ok("load-acl", morning), morning)
