...
class Router(object):
	...
	def normalize(self):
		self.commands=[]
		self.normalizeRequired=False
		if not "trigger-test-1" in self.acls:
			self.normalizeRequired=True
			self.commands+=["ip access-list standard trigger-test-1","permit 1.1.1.1"]
			print "{}: Will normalized trigger-test acl on device".format(self.name)

		pre_commands=["conf t"]
		post_commands=["end","write mem"]
		self.commands=pre_commands+self.commands+post_commands
		# self.commands=map(lambda x:x.encode("ascii"),self.commands)
